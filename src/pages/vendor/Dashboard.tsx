...
const VendorDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch vendor's jobs with applications
      const { data: jobsData, error: jobsError } = await supabase
        .from("jobs")
        .select("*, job_applications(*, profiles(*))")
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="grid gap-6 md:grid-cols-3 mb-8">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {jobs.reduce((acc, job) => acc + (job.job_applications?.length || 0), 0)}
              </div>
                ...
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <CardDescription>{job.work_type}</CardDescription>
                    </div>
                    <Badge
                      variant={
                        job.status === "open"
                ...
                <CardContent>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-sm font-semibold">
                      Budget: ${job.pay_min} - ${job.pay_max}
                    </span>
                    <span className="text-sm text-muted-foreground">
                      {job.job_applications?.length || 0} application(s)
                    </span>
                  </div>
                          ...
                          >
                            <div>
                              ...
                              <div className="flex gap-2 mt-1">
                                ...
                                <Badge variant="secondary" className="text-xs">
                                  ⭐ {application.profiles.rating?.toFixed(1) || "0.0"}
                                </Badge>
                              </div>
                            </div>
                            <div className="flex gap-2">
